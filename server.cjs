...
function enrichCompdetSalePrintGlobals(row) {
  ...
  row.G_CIN_NO = tv('cin_no') || tv('CIN_NO');
  ...
  row.G_MSME_NO = tv('msme_no') || tv('MSME_no') || tv('MSME_NO');
  ...
  try {
    ...
    const sql = `
      ...
      LEFT JOIN ITEMMAST C
        ON A.COMP_CODE = C.COMP_CODE
       AND TRIM(TO_CHAR(A.ITEM_CODE)) = TRIM(TO_CHAR(C.ITEM_CODE))
      LEFT JOIN MASTER D ON A.COMP_CODE = D.COMP_CODE AND TRIM(TO_CHAR(A.B_CODE)) = TRIM(TO_CHAR(D.CODE))

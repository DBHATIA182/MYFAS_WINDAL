  ...
  const handleSave = async (saveMode) => {
    ...
    try {
      const payload = {
        ...
        r_date: rDateOracle,
        r_no: saveMode === 'add' ? undefined : rNo,
        ...
        <div className="dc-header-row dc-header-row--party">
          ...
          <div className="dc-header-row__body">
            {partyInfo && !partyFinderOpen ? (
              ...
              <div className="dc-party-find">
                <input
                  className="form-input sale-bill-search-input"
                  placeholder="Search party — code, name, city (schedule 11.20)"
                  value={partySearch}
                  disabled={fieldsDisabled}
                  onChange={(e) => {
                ...
                />
                {partySearch.trim() ? (
                  <ul className="account-search-list dc-party-list" role="listbox">
                    {filteredParties.slice(0, 80).map((p, i) => {
                      const pc = String(p.CODE ?? p.code);
                      return (
                        <li
                          key={pc}
                          role="option"
                          className={i === partyHi ? 'account-search-item--hi' : ''}
                          onMouseEnter={() => setPartyHi(i)}
                          onClick={() => {
                            setCode(pc);
                            setPartyFinderOpen(false);
                            setPartySearch('');
                        ...
                        >
                          ...
                          {highlightMatch(p.CITY ?? p.city, partySearch)}
                        </li>
                      );
                    })}
                  </ul>
            ...
            <tbody>
              {lines.map((L, idx) => (
                <tr key={idx}>
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        applyItemToLine(idx, v);
                        recalcLine(idx, { item_code: v });
                      }}
                    />
                    <datalist id={`dc-items-${idx}`}>
                      {(lookups.items || []).slice(0, 200).map((it) => (
                        <option key={it.ITEM_CODE ?? it.item_code} value={String(it.ITEM_CODE ?? it.item_code ?? '')} />
                      ))}
                    </datalist>
                  </td>
                  ...
                  <td>
                    <input readOnly className="sale-bill-input-readonly dc-num dc-num--amt" value={fmtAmt(L.amount)} />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <section className="sale-bill-section sale-bill-section--card">
        ...
        <div className="sale-bill-totals-grid">
          <label className="sale-bill-field sale-bill-field--block">
            ...
            <input value={remarks} disabled={fieldsDisabled} onChange={(e) => setRemarks(e.target.value)} />
          </label>
          <label className="sale-bill-field sale-bill-field--block">
            <span className="sale-bill-field__label">Truck no</span>
            <input value={truckNo} disabled={fieldsDisabled} onChange={(e) => setTruckNo(e.target.value)} />
          </label>
          <label className="sale-bill-field sale-bill-field--block">
            <span className="sale-bill-field__label">Transport</span>
            <input value={tpt} disabled={fieldsDisabled} onChange={(e) => setTpt(e.target.value)} />
          </label>
          <label className="sale-bill-field sale-bill-field--block">
            <span className="sale-bill-field__label">GR no</span>
            <input value={grNo} disabled={fieldsDisabled} onChange={(e) => setGrNo(e.target.value)} />
          </label>
        </div>
      </section>

...
export default function GrnListScreen({
  ...
  const runReport = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${apiBase}/api/grn-list-report`, { params, ...reqOpts });
  ...
  const listActionButtons = (
    <>
      ...
      <button
        ...
        disabled={!hasRows}
        onClick={() =>
          generatePDF('grn-list', { rows: reportRows }, pdfMeta).catch((e) =>
      ...
      <button
        ...
        disabled={!hasRows}
        onClick={() => downloadExcelRows(excelRows, 'GRN', `${compName}_GRN_List`)}
      >
        Excel
      </button>
      <button
        ...
        onClick={() =>
          sharePdfWithWhatsApp('grn-list', { rows: reportRows }, pdfMeta, shareText).catch((e) =>
            alert(e?.message || String(e))
          )
        }
      >
  ...
  return (
    <div className="slide slide-22-grn-list dc-list-screen">
      <header className="dc-list-screen__head">
        <h2 className="sale-bill-page__title">Goods receipt note list</h2>
      </header>
      ...
      {ran ? (
        <section className="sale-bill-section sale-bill-section--card dc-list-results">
          ...
          <p className="sale-bill-section__hint">Click a row to open that goods receipt note in the entry screen.</p>
        </section>
      ) : null}

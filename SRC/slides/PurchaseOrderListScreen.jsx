...
export default function PurchaseOrderListScreen({
  ...
  const parties = lookups?.suppliers || [];
  ...
  const runReport = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${apiBase}/api/purchase-order-list-report`, { params, ...reqOpts });
  ...
  const listActionButtons = (
    <>
      ...
      <button
        ...
        disabled={!hasRows}
        onClick={() =>
          generatePDF('purchase-order-list', { rows: reportRows }, pdfMeta).catch((e) =>
      ...
      <button
        ...
        disabled={!hasRows}
        onClick={() => downloadExcelRows(excelRows, 'PurchaseOrder', `${compName}_PurchaseOrder_List`)}
      >
        Excel
      </button>
      <button
        ...
        onClick={() =>
          sharePdfWithWhatsApp('purchase-order-list', { rows: reportRows }, pdfMeta, shareText).catch((e) =>
            alert(e?.message || String(e))
          )
        }
      >
  ...
  return (
    <div className="slide slide-24-purchase-order-list dc-list-screen">
      <header className="dc-list-screen__head">
        <h2 className="sale-bill-page__title">Purchase order list</h2>
      </header>
      ...
      <section className="sale-bill-section sale-bill-section--card dc-list-filters">
        ...
        <div className="dc-list-filters-grid">
          ...
          <label className="sale-bill-field sale-bill-field--block">
            <span className="sale-bill-field__label">Specific supplier</span>
            <input
      ...
      {ran ? (
        <section className="sale-bill-section sale-bill-section--card dc-list-results">
                ...
                {reportRows.length === 0 ? (
                  ...
                  reportRows.map((r, i) => (
                    <tr
                      ...
                      title="Open this purchase order in entry"
                      onClick={() =>
          ...
          <p className="sale-bill-section__hint">Click a row to open that purchase order in the entry screen.</p>
        </section>
      ) : null}

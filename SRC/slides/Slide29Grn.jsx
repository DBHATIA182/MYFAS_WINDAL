...
import GrnListScreen from './GrnListScreen';
import GrnPrintScreen from './GrnPrintScreen';
...
function focusLineQnty(lineIdx) {
  window.requestAnimationFrame(() => {
    const root = document.querySelector('.slide-29-grn');
...
function handleEnterAsTab(e) {
  ...
  const t = e.target;
  if (!t || t.closest('.slide-29-grn-ignore-enter')) return;
  if (t.tagName === 'TEXTAREA') return;
  e.preventDefault();
  const root = t.closest('.slide-29-grn');
...
function DispatchPickModal({ open, title, hint, emptyMessage, loading, rows, columns, hi, onHi, onClose, onPick }) {
  ...
  return createPortal(
    <div className="sale-bill-pick-overlay slide-29-grn-ignore-enter" role="presentation" onClick={onClose}>
      <div
        ref={cardRef}
...
export default function Slide29Grn({ apiBase, formData, userName, onPrev, onReset }) {
  const rNoRef = useRef('');
  ...
  const loadBootstrap = useCallback(async () => {
    ...
    try {
      ...
      const [pRes, cRes, lRes, mpRes] = await Promise.all([
        axios.get(`${apiBase}/api/grn-user-permissions`, { params, ...reqOpts }),
        axios.get(`${apiBase}/api/grn-form-context`, {
          params: {
            comp_code: compCode,
            comp_uid: compUid,
        ...
        axios.get(`${apiBase}/api/grn-lookups`, {
  ...
  const fetchNextRNo = useCallback(async () => {
    ...
    const { data } = await axios.get(`${apiBase}/api/grn-next-r-no`, {
  ...
  const loadBySlot = useCallback(
    async (targetRNo, targetChType) => {
      ...
      try {
        const { data } = await axios.get(`${apiBase}/api/grn-raw`, {
          params: {
            comp_code: compCode,
            comp_uid: compUid,
  ...
  const openSoPick = async (lineIdx) => {
    ...
    if (!code) {
      showNotice('Select supplier before pending PO (F1).');
      return;
    }
    setSoPick({ open: true, lineIdx, rows: [], loading: true, hi: 0 });
    try {
      const { data } = await axios.get(`${apiBase}/api/grn-pending-orders`, {
  ...
  const soPickColumns = useMemo(
    () => [
      { key: 'so', label: 'PO no', render: (r) => displayLineInt6(r.SO_NO ?? r.PO_NO) },
  ...
  const handleSave = async (saveMode) => {
    ...
    if (!code) {
      showNotice('Select supplier.');
      return;
    }
    if (fyMinYmd && fyMaxYmd && rDateYmd && (rDateYmd < fyMinYmd || rDateYmd > fyMaxYmd)) {
      showNotice(`GRN date must be between ${toDisplayDate(fyMinYmd)} and ${toDisplayDate(fyMaxYmd)}.`);
      return;
    }
    ...
    try {
      ...
      const { data } = await axios.post(`${apiBase}/api/grn-save`, payload, reqOpts);
      if (saveMode === 'delete') {
        setMsg('Goods receipt note deleted.');
        setPostedNew(false);
        setMode('new');
        clearForNew();
      } else {
        ...
        setMode('edit');
        setMsg(saveMode === 'add' ? 'Goods receipt note saved.' : 'Goods receipt note updated.');
      }
    } catch (e) {
  ...
  if (listScreenOpen) {
    return (
      <GrnListScreen
        ...
        onOpenGrn={(row) => void openChallanFromList(row)}
      />
    );
  }
  ...
  if (loading) {
    return (
      <div className="slide slide-29-grn slide-29-grn--loading">
        <div className="sale-bill-loading-card">
          <h2 className="sale-bill-page__title">Goods receipt note</h2>
  ...
  if (!can.canOpen) {
    return (
      <div className="slide slide-29-grn">
        <h2 className="sale-bill-page__title">Goods receipt note</h2>
  ...
  return (
    <div
      className="slide slide-22-dispatch-challan slide-29-grn sale-bill-page sale-entry-desktop"
    ...
    >
            ...
            <tbody>
              {lines.map((L, idx) => (
                <tr key={idx}>
                  <td>{idx + 1}</td>
                  <td>
                    <div className="dc-line-so-cell slide-29-grn-ignore-enter">
                      <button
                        type="button"
                        className="dc-line-pick-mobile slide-29-grn-ignore-enter"
                      ...
                      <button
                        ...
                        disabled={!canEditLines}
                        title="Pick pending PO (F1)"
                        aria-label="Pick pending PO"
                        onClick={() => void openSoPick(idx)}
                      >
      ...
      <DispatchPickModal
        ...
        emptyMessage="No pending PO for this party."
